...
Questo documento riassume il procedimento utilizzato per estrarre i valori dell'NDVI per ogni pixel all'interno dei poligoni delle chiome per almeno i 2/3 della propria area.
...
# Supponendo che "crowns" sia il tuo oggetto sf
crowns <- crowns %>%
  group_by(AREA) %>%
...
for (i in 1:length(raster_files)) {
  shp0 <- vect(shapefile_files[7])
  rast <- terra::rast(raster_files[7])
  ...
  area_name <- tools::file_path_sans_ext(basename(shapefile_files[7]))
...
Si modifica il nome della colonna `value` in `ndvi` per ogni oggetto sf della lista. Sucessivamente si crea un file unico contente tutti gli oggetti della lista e lo si salva in locale.

```{r, eval = FALSE}
shapefile_folder  <- "G:/Altri computer/Il_mio_computer/DOTTORATO/PROGETTI/OLIVASTRO_PAULILATINO/REGRESSIONE/VETTORIALI/crowns_areas"

# Modifica il nome della colonna value in ndvi per ciascun oggetto sf nella lista
...
```{r, eval = FALSE}
ndvi_avg_areas1 <- st_read("G:/Altri computer/Il_mio_computer/DOTTORATO/PROGETTI/OLIVASTRO_PAULILATINO/REGRESSIONE/VETTORIALI/NDVI_VALUES/ndvi_avg_areas.shp")
...
# Merge the datasets based on date and AREA
ndvi_avg_areas2 <- ndvi_avg_areas1 %>%
  st_join(crowns, by = c("date", "AREA")) %>%
  select(date, AREA = AREA.x, mean_NDVI, ID, CODICE_)

# Set the geometry column from crowns as the active one
st_geometry(ndvi_avg_areas2) <- "geometry.y"

# Salva l'oggetto sf combinato nel file shapefile
st_write(ndvi_avg_areas2, "G:/Altri computer/Il_mio_computer/DOTTORATO/PROGETTI/OLIVASTRO_PAULILATINO/REGRESSIONE/VETTORIALI/NDVI_VALUES/ndv_areas2.shp", append= FALSE)
```
...
ndvi_avg <- st_read("G:/Altri computer/Il_mio_computer/DOTTORATO/PROGETTI/OLIVASTRO_PAULILATINO/REGRESSIONE/VETTORIALI/NDVI_VALUES/ndv_areas2.shp")
...
crowns <- st_read("G:/Altri computer/Il_mio_computer/DOTTORATO/PROGETTI/OLIVASTRO_PAULILATINO/REGRESSIONE/VETTORIALI/crowns_merged.shp")
...
st_write(pixel_trend, "G:/Altri computer/Il_mio_computer/DOTTORATO/PROGETTI/OLIVASTRO_PAULILATINO/REGRESSIONE/VETTORIALI/NDVI_VALUES/pixels_trend.shp")
```
...
st_write(pixel_trend, "G:/Altri computer/Il_mio_computer/DOTTORATO/PROGETTI/OLIVASTRO_PAULILATINO/REGRESSIONE/VETTORIALI/NDVI_VALUES/pixels_trend2.shp")
```
...
map <- leaflet(options = leafletOptions(maxZoom = 22)) %>%
  ...
  addPolygons(data = pixel_trend, fillColor = ~color_factor(clss_tr),
              fillOpacity = 0.6,
